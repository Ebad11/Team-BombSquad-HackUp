...
def get_brand_features(hostname, registered_domain, subdomain, url):
    ...
    try:
        ...
        dist_func = lev.distance
    except ImportError:
        dist_func = levenshtein_dist

    min_lev = min(
        (dist_func(reg_clean, brand) for brand in TOP_BRANDS
...
def predict():
    ...
    is_email = data.get("is_email", False) or "sender_email" in data or "has_attachment" in data
    ...
    if is_email:
        ...
        # 1. Text Model Score
        if text:
            p_text, _, _ = stage2_text_ml(text)
            factors["text_score"] = round(p_text, 4)
        else:
            factors["text_score"] = 0.0
            
        # 2. URL Phishyness
        if url:
            url_res = run_pipeline(url, "")
            factors["url_score"] = round(url_res.get("probability", 0.0), 4) if "error" not in url_res else 0.0
        else:
            factors["url_score"] = 0.0
            
        # 3. Attachment Score
        factors["attachment_score"] = 0.5 if has_attachment else 0.0
        
        # 4. Impersonation Score
        ...
        if sender_email and '@' in sender_email:
            ...
            if domain not in TRUSTED_DOMAINS:
                ...
                for brand in TOP_BRANDS:
                    if abs(len(domain_clean) - len(brand)) <= 3:
                        d = dist_func(domain_clean, brand)
                        if 0 < d <= 2 and len(domain_clean) >= 4:
                            impersonation_score = 0.95
                            impersonated_brand = brand
                            break
                            
        factors["impersonation_score"] = impersonation_score
        
        # Aggregate Score
        w_text = 0.40 if text else 0.0
        w_url = 0.30 if url else 0.0
        w_att = 0.10 if has_attachment else 0.0
        w_imp = 0.20 if sender_email else 0.0
        
        total_weight = w_text + w_url + w_att + w_imp
        
        if total_weight > 0:
            agg_score = sum(factors[k] * w for k, w in zip(["text_score", "url_score", "attachment_score", "impersonation_score"], [w_text, w_url, w_att, w_imp])) / total_weight
        else:
            agg_score = 0.0
        ...
        if has_attachment and (factors["text_score"] > 0.6 or factors["url_score"] > 0.6):
            agg_score = max(agg_score, 0.75)
            
        agg_score = round(min(agg_score, 1.0), 4)
        
        conf_pct, conf_label, is_uncertain = stage7_confidence(agg_score)
        label = stage8_classify(agg_score, is_uncertain)
        ...
        history.appendleft({
            "url": sender_email[:70] if sender_email else "Direct Email",
            "label": label,
            "prob": agg_score,
            "conf": conf_label,
