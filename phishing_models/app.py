...
import os, re, io, math, json, pickle, logging, warnings
...
def predict_url(url: str) -> dict:
    ...
    if xgb_prob is not None:
        prob = (xgb_prob * 0.65 + lr_prob * 0.35)
    else:
        prob = lr_prob

    signals = []
    if feats["has_ip"]:        signals.append("IP address used instead of domain")
    if feats["is_suspicious_tld"]: signals.append(f"Suspicious TLD detected")
...
def parse_email(
    subject: str,
    body: str,
    attachments: list[dict],  # [{"name": str, "bytes": bytes}]
) -> dict:
    """
...
──────────────────────
score = w_url * max(url_probs)   [worst URL wins]
      + w_text_lr * lr_prob
      + w_text_bert * bert_prob  (if available)
      + attachment_penalty

Weights are empirically chosen; tune as needed.
"""
W_URL        = 0.40
W_TEXT_LR    = 0.30
W_TEXT_BERT  = 0.25
...
def run_risk_engine(
    ...
    components["text_bert"] = round(bert_score, 4) if bert_score else None
    ...
    if bert_available:
        ...
        # Redistribute BERT weight to LR and URL
    ...
    # Boost if multiple high-confidence signals align
    high_signals = sum([
        url_score  > 0.7,
        lr_score   > 0.7,
        (bert_score or 0) > 0.7,
    ...
    return {
        ...
        "all_signals":    list(dict.fromkeys(all_signals)),  # dedup
...
def predict_url_endpoint():
    ...
    Output (JSON): URL phishing analysis + risk tier
    """
...
def predict_email_endpoint():
    ...
    Output (JSON): full email analysis with risk engine output
    """
    ...
    for f in request.files.getlist("attachments[]"):
        ...
        if ext in ALLOWED_ATTACH:
            attachments.append({"name": fname, "bytes": f.read()})

    # ── Decompose email ───────────────────────────────────────────────────────
    parsed = parse_email(subject, body, attachments)

    # ── Run URL model on every extracted URL ──────────────────────────────────
    url_results = []
    for url in parsed["urls"][:20]:   # cap at 20 URLs per email
        url_results.append(predict_url(url))

    # ── Run text model on email body ──────────────────────────────────────────
